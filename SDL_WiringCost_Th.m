function SDL_WiringCost_Th(SDL)
...
if strcmp(SDL.ana_type{1},'med') || strcmp(SDL.ana_type{1},'medcov')% mediation matrix
    ...
    D = MMc(k+1:end,:,1);  [Thr(2,1), WC(2,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g2
    W.WC_abs = WC; W.Thr_abs = Thr;
...
save(fn,'W','-append','-v7.3'); fprintf('Saved: Minimum Wiring Cost & Corresponding Thresholds in %s\n',fn);
