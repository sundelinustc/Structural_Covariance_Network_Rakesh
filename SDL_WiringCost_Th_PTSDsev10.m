function SDL_WiringCost_Th_PTSDsev10(SDL)
...
if strcmp(SDL.ana_type{1},'med') || strcmp(SDL.ana_type{1},'medcov')% mediation matrix
    ...
    k = 0.2 * size(MMc,1); % each group has 148 regions
    % abslute values
    Thr = []; WC = []; ftype = 'abs';
    D = MMc(    1:k,  :,1);  [Thr(1,1), WC(1,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g1
    D = MMc(  k+1:2*k,:,1);  [Thr(2,1), WC(2,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g2
    D = MMc(2*k+1:3*k,:,1);  [Thr(3,1), WC(3,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g3
    D = MMc(3*k+1:4*k,:,1);  [Thr(4,1), WC(4,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g4
    D = MMc(4*k+1:5*k,:,1);  [Thr(5,1), WC(5,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g5
    W.WC_abs = WC; W.Thr_abs = Thr;
