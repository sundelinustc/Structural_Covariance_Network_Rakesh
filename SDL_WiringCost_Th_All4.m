function SDL_WiringCost_Th_All4(SDL)
...
if strcmp(SDL.ana_type{1},'med') || strcmp(SDL.ana_type{1},'medcov')% mediation matrix
    ...
    k = 0.25 * size(MMc,1); % each group has 148 regions
    % abslute values
    Thr = []; WC = []; ftype = 'abs';
    ...
    D = MMc(2*k+1:3*k,:,1);  [Thr(3,1), WC(3,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g21
    D = MMc(3*k+1:4*k,:,1);  [Thr(4,1), WC(4,1)] = SDL_ThresMinEdges(D,ftype,atype); % for g22
    W.WC_abs = WC; W.Thr_abs = Thr;
